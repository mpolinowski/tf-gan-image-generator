...
# import keras cifar100 dataset (50000 labeled images 32x32)
...
# # visualize dataset
# plt.imshow(train_images[45999].reshape((32,32)) , cmap = 'gray')
# plt.show()
...
def make_generator_model():
    ...
    ## Because we used "same" padding and stride = 1, the output is the same size as input 8 x 8 but with 128 filters instead
    model.add(layers.Conv2DTranspose(128, (5, 5), strides=(1, 1), padding='same', use_bias=False)) 
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    ## Because we used "same" padding and stride = 2, the output is double the size of the input 16 x 16 but with 64 filters instead
    model.add(layers.Conv2DTranspose(64, (5, 5), strides=(2, 2), padding='same', use_bias=False))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    ## Because we used "same" padding and stride = 2, the output is double the size of the input 32 x 32 but with 1 filter instead
...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv2D(32, (3, 3), strides=(2, 2), padding='same', input_shape=[32, 32, 3]))
    model.add(layers.LeakyReLU())
...
checkpoint_dir = './cifar100_training_checkpoints'
...
def train(dataset, epochs):
  for epoch in range(epochs):
    ...
    ## create checkpoint every 50 epochs
    if (epoch + 1) % 50 == 0:
